...
export function useMeasurementLines(getLineLengthInFeet) {
  ...
  const removeMeasurementLine = useCallback(
    (lineId) => {
  ...
  const findMeasurementLineAtPoint = useCallback(
    (x, y, tolerance = 20) => {
  ...
  const findMeasurementHandleAtPoint = useCallback(
    (x, y, tolerance = 15) => {
      if (!selectedMeasurementId) return null;

      const selectedLine = measurementLines.find(
        (line) => line.id === selectedMeasurementId
      );
  ...
  const recalculateAllMeasurements = useCallback(() => {
    ...
    console.log(
      "📏 MeasurementLines: Recalculating all measurements due to reference scale change"
    );
